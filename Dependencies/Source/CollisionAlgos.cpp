...
namespace collisionAlgos {
    CollisionPoint checkCollision(Sphere* bv1, Sphere* bv2) {
        ...
        if(glm::length(center2centerVec) <= bv1->radius + bv2->radius) {
            ...
            return CollisionPoint(bv1->getCurrentPosition() - center2centerVec * bv1->radius, bv2->getCurrentPosition() + center2centerVec * bv2->radius, bv1, bv2);
        }
        return {};
    }
...
//                                    std::max(newMin1.z, std::min(newMax2.z, newMax1.z)));
        ...
        //std::cout << "Coliziune\n";
        CollisionPoint p(bv1->closestPoint(mean), bv2->closestPoint(mean), bv1, bv2);
        //std::cout << p.toString() << "\n";
        return p;
    }
    ...
    CollisionPoint checkCollision(AABB* bv1, Sphere* bv2) {
        ...
        if (glm::distance(v, O2) <= bv2->radius) {
            if (O2 - v == glm::vec3(0)) {
                return {bv1->closestPoint(v), O2 - bv2->radius * glm::normalize(O2 - (newMin + newMax) / 2.0f), bv1, bv2 };
            } else {
                return {bv1->closestPoint(v), O2 - glm::normalize(O2 - v) * bv2->radius, bv1, bv2 };
            }
        }
        return {};
    }
    ...
    CollisionPoint checkCollision(Sphere* bv1, Ray* bv2) {
        ...
        if (delta == 0) {
            ...
            if (fabs(distance1 + distance2 - bv2->length) <= EPS) {
                std::cout << "brah0\n";
                return CollisionPoint(solution.point, solution.point, bv1, bv2); // return true;
            }
        } else {
            Solution solution1{};
            ...
            */
            ...
            return CollisionPoint(start, end, bv1, bv2);
    ...
    CollisionPoint checkCollision(Triangle* bv1, Ray* bv2) {
        ...
        if (glm::dot(bv1->norm, B - A) == 0) {
            if (bv1->isInside(A))
                return { A, A + glm::vec3(0.01f), bv1, bv2 };
            if (bv1->isInside(B))
                return { B, B + glm::vec3(0.01f), bv1, bv2 };
        }
        ...
        if (bv1->isInside(solution.point))
            return { solution.point, solution.point + glm::vec3(0.01f), bv1, bv2 };
...
namespace collisionAlgos {
    ...
    bool hasCollision(Triangle* bv1, Triangle* bv2) {
        ...
        bool result = r0.hasCollision(bv1) || r1.hasCollision(bv1) || r2.hasCollision(bv1);
    ...
    bool hasCollision(AABB* bv1, Triangle* bv2) {
        ...
        return r0.hasCollision(bv1) || r1.hasCollision(bv1) || r2.hasCollision(bv1);
    }
    bool hasCollision(AABB* bv1, Capsule* bv2) {
        glm::vec3 bestPoint = ClosestPointOnLineSegment(bv2->getStart(), bv2->getEnd(), (bv1->getMin() + bv1->getMax()) / 2.0f);
        Sphere s(bestPoint, bv2->radius);
        return s.hasCollision(bv1);
    }
    ...
    bool hasCollision(OBB* bv1, Sphere* bv2) { return{}; }
    bool hasCollision(OBB* bv1, OBB* bv2) { return{}; }
    bool hasCollision(OBB* bv1, Ray* bv2) { return{}; }
    bool hasCollision(OBB* bv1, Capsule* bv2) { return{}; }
    bool hasCollision(OBB* bv1, Triangle* bv2) { return{}; }
    ...
    bool hasCollision(Sphere* bv1, Triangle* bv2) {
        ...
        return bv1->hasCollision(&r0) || bv1->hasCollision(&r1) || bv1->hasCollision(&r2);
    }
    ...
    bool hasCollision(Triangle* bv1, Capsule* bv2) {
        ...
        return bv2->hasCollision(&r0) || bv2->hasCollision(&r1) || bv2->hasCollision(&r2);
    }
